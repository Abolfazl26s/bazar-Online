(function ($) {
  ...
  $(function () {
    // =============== location Selector ================
    $(window).on("load", () => {
      $.get("./json/provinces.json", (data, status) => {
        let provinces = [];
        if (status == "success") {
          ...
          $("#provinces").append(provinces.join(""));
        }
      });
    });
    ...
    $("#provinces").change(function (e) {
      e.preventDefault();
      let selectedprovince = $(this).children("option:selected").val();
      let cities = [];
      // console.log(selectedprovince);

      $.get("../../json/cities.json", (data, status) => {
        // console.log(status);

        $.each(data, function (key, value) {
          if (selectedprovince == value.province_id) {
            cities.push(
              `
                <option value='${value.id}'>
                  ${value.name}
        ...
        $("#cities").html(`<option value="0">انتخاب کنید</option>`);
        cities.push(
          `
                <option value='other'>
                  درلیست وجود ندارد
        ...
        $("#cities").append(cities.join(""));
      });
    });
  });

  // ===============================
  // active icon in navMobile
  // ===============================

  $(".navItem:not(.btnCategories)").click(function () {
...
$(function () {
  ...
  $(".add-to-cart").click(function (e) {
    e.preventDefault();
    let nameProduct = $(this).parents(".item").find(".product_title");
    toastr.success(`${nameProduct.text()} به سبد اضافه شد`);
  });

  $(".add-compare").click(function (e) {
    e.preventDefault();
    let nameProduct = $(this).parents(".item").find(".product_title");
    toastr.warning(`${nameProduct.text()} به لیست مقایسه اضافه شد`);
  });

  $(".btnReaustProduct").click(function (e) {
    e.preventDefault();
    toastr.success(`در خواست شما با موفقیت ثبت شد`);
